...
class Camera:
    ...
    def detect_aruco_target(self, frame, target_id, aruco_dict_type=cv2.aruco.DICT_5X5_250):
    
        if self.calibration_matrix is None or self.dist_coeffs is None:
            print("please set calibration file")
            return
        
        output_frame, pose = self.target_pose_estimation(frame, aruco_dict_type, self.calibration_matrix, self.dist_coeffs, target_id)

        return pose, output_frame

    def target_pose_estimation(self, frame, aruco_dict_type, matrix_coefficients, distortion_coefficients, target_id):
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        cv2.aruco_dict = cv2.aruco.Dictionary_get(aruco_dict_type)
        parameters = cv2.aruco.DetectorParameters_create()
